...
class Database:
    ...
    def __init__(self):
        ...
        self.cr.execute('CREATE TABLE IF NOT EXISTS `search_results` ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, `search_id` INTEGER NOT NULL, `title` TEXT, `description` TEXT, `content` TEXT, `image` TEXT, `is_saved` INTEGER, `author` TEXT)')
        self.db.commit()
    ...
    def inset_search_result(self, search_id: tuple, search_results: dict):
        try:
            for search_result in search_results.get('articles'):
                self.cr.execute('INSERT INTO search_results VALUES(null,?, ?, ?, ?, ?, 0, ?)', [search_id[0], search_result['title'], search_result['description'], search_result['content'], search_result['urlToImage'], search_result['author']])
            self.db.commit()
